    ...
    if (mode == 1) {
      ...
      for (int i=0; i<buttons2.length; i++) {
        ...
        if (clicked2[i]) {  
          ...
          controller.sendSignal(this.onTime, this.offTime);
          if (controllerState != controller.getState()) {
    ...
    if (mode == 1) { 
      for (int i=0; i<buttons.length; i++) {
        ...
        if (buttons[i].checkMouse()) { 
          ...
          if (controller.getCurValI(i) == 1) {
            controller.setCurValI(i, 0);
          } else if (controller.getCurValI(i) == 0) {
            controller.setCurValI(i, 1);
          }
        }
      }
